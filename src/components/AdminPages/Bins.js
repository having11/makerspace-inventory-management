...
import BinAdd from './Forms/Add/BinAdd'
...
const columns = [
    ...
    {id: 'parent_container', label: 'Container ID', minWidth: 50},
    {id: 'row', label: 'Row', minWidth: 50},
    {id: 'col', label: 'Column', minWidth: 50},
    {id: 'size', label: 'Size (cm)', minWidth: 50},
...
class Bins extends React.Component {
    ...
    getCategoryRows() {
        axios.get(`http://${config.serverIP}:${config.serverPort}/inv_sys/api/bin/all`)
        .then((response) => {
            ...
            {(this.state.loaded) ? 
                ...
                >
                    <div style={this.state.modalStyle} className={this.classes.modalPaper}>
                        <BinAdd whenSubmitted={this.catCreated} />
                    </div>
...
export default withStyles(styles)(Bins);